...
from word_convert import word_to_pdf


class PDFToolApp(tk.Toplevel):
    ...
    def _save_row(self, parent, label, variable, filetypes):
        ...
        def pick():
            path = filedialog.asksaveasfilename(parent=self,defaultextension=".pdf", filetypes=filetypes)
            if path:
                variable.set(path)
